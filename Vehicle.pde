abstract class Vehicle {
  ...
  Vehicle(float x, float y, float r, float ms, float mf, float t) {
    ...
    desired = position.copy();                                           //creating the desired velocity as a zero
  ...
  Vehicle display(){
    ...
    if(type == 0)image(tree, -radius/2,-radius/2,radius,radius);         //if statements to determine which image to draw
  ...
  Vehicle update() {
    forward = velocity.copy().normalize();                               //set the forward to the normal of the velocity
