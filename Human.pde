class Human extends Vehicle {
  ...
  Human(float x, float y, float r, float ms, float mf, boolean o, float t){           //contructor takes all of the same params as the vehicle class so they are all passed onto super
    ...
    fill(#ffffff);
    if(o)
      body = createShape(RECT, 0,0,r,r);                                              //create the shape as a rectangle if it is to be an obstacle
    else
      body = createShape(ELLIPSE, 0,0,30,30);                                         //create the shape as a circle if it is to be a human
  ...
  PVector wander(){
    ...
    return position.copy()                                                              //move from current position to the center of the circle to the position on the circle based on the rotation
